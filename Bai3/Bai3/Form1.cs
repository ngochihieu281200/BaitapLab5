...
namespace Bai3
{
    public partial class FormBai3 : Form
    {
        ...
        {
            NhanVien nhanVien;
            string msnv =this.txtBoxHTNV.Text;
            string hoTen = this.txtBoxHTNV.Text;
            ...
            if(isNhanVienKinhDoanh)
            {
                ...
                nhanVien = new NVKinhDoanh(msnv, hoTen, luongCoBan, soLuongHopDong);
            }
            else
            {
                int soLuongSanPham = int.Parse(this.txtBoxSLSP.Text);
                nhanVien = new NVSanXuat(msnv, hoTen, soLuongSanPham);
            }
            this.lblLuongNhanVien.Text = nhanVien.TinhLuong().ToString();

        }
        private void clearTexBoxNVSx_And_NVKD()
        {
